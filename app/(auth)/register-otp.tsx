...
export default function RegisterOtpScreen() {
  ...
  const { verifyRegisterOtp, sendRegisterOtp } = useAuth();
  ...
  const handleVerify = async (code?: string) => {
    ...
    setLoading(true);
    try {
      // Stores registrationToken in-memory inside AuthContext
      await verifyRegisterOtp(phone!, otpCode);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => {});
      router.replace({ pathname: '/(auth)/register-details', params: { phone } });
    } catch (err: any) {
      ...
      if (err?.status === 429) {
        // Too many wrong attempts — OTP invalidated, must restart
